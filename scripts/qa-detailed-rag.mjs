...
function buildReport(suite, mode, results, startedAt, elapsedMs) {
  ...
  const report = {
    ...
    leakagePolicy: {
      ...
      holdoutCanTuneAliasesAndEvidence: false,
      adversarialCanTuneGuardsOnly: true,
    },
  };
...
async function main() {
  ...
  const semanticDocument = options.mode === 'retrieval' && options.semanticPath
    ? JSON.parse(await readFile(resolve(options.semanticPath), 'utf8'))
    : null;
  if (semanticDocument && (semanticDocument.suiteVersion !== suite.suiteVersion || semanticDocument.kbVersion !== suite.kbVersion)) {
    throw new Error('Query embeddings do not match the Q&A suite and knowledge-base versions.');
  }
  ...
  const report = buildReport(suite, options.mode, results, startedAt, performance.now() - started);
